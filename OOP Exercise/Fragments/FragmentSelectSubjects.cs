...
namespace OOP_Exercise.Fragments
{
    public class FragmentSelectSubjects : Fragment
    {
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            ...
            {
                subjectList = (from item in LoginDataManager.Scheduler[0].tkb where !item.ten_mh.Contains("(mr") && !item.ten_mh.Contains("(th") && !item.ten_mh.Contains("(bt") select item.ten_mh).ToList();
            }
